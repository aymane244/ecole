...
function FancyTable($header)
{
    ...
    $this->SetFont('','B');
    // En-tête
    $w = array(5, 38, 7, 17 , 25, 55, 64, 22, 17, 30);
    ...
    foreach ($etudiants as $etudiant) {
        ...
        $this->Cell($w[6],6,$etudiant['etud_adress'],'LR',0,'L',$fill);
        $this->Cell($w[7],6,$etudiant['etud_permis'],'LR',0,'L',$fill);
        $this->Cell($w[8],6,$etudiant['etud_cat_permis'],'LR',0,'C',$fill);
        $this->Cell($w[9],6,$etudiant['etud_permis_obt'],'LR',0,'L',$fill);
        $this->Ln();
        $fill = !$fill;
        }
    }
...
$pdf->AliasNbPages();
// Titres des colonnes
$header = array('#', 'Nom complet', 'Age', 'CIN', iconv('UTF-8', 'windows-1252', 'Téléphone'), 'Email', 'Adresse', 'Permis', iconv('UTF-8', 'windows-1252', 'Catégorie'), 'Date d\'obtention');
